...
import com.peixoto.usuario.infrastructure.security.RedisTokenBlacklistService;
...
public class UsuarioController {
    ...
    public ResponseEntity<Void> logout(HttpServletRequest request, HttpServletResponse response) {
        if (request.getCookies() != null) {
            for (Cookie cookie : request.getCookies()) {
                if ("pnp_token".equals(cookie.getName())) {
                    redisTokenBlacklistService.blacklist(cookie.getValue());
                    break;
                }
            }
        }
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            redisTokenBlacklistService.blacklist(authHeader.substring(7).trim());
        }
